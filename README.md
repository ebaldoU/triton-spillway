...
```bash
cd spillway
TRITON_BASE_URI=/ruta/a/datos/triton_results \
...
cd spillway
TRITON_BASE_URI=/ruta/a/datos/triton_results \
TRITON_GTIFF_DIR=/ruta/a/datos \
  python geotiff_to_tiledb_sparse.py --dataset datos1
```

Tarda ~10 minutos por dataset con un pico de ~4 GB de RAM. Al finalizar verifica automáticamente la integridad del array.

### Verificar integridad

```bash
python verify_tiledb.py --dataset datos1 --n 1000
```
...
```bash
# Benchmark TileDB vs GeoTIFF (requiere GeoTIFFs fuente)
python benchmark_vs_geotiff.py --dataset datos1

# Benchmark de patrones de consulta

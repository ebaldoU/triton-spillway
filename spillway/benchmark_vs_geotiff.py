...
def main():
    ...
    if have_gtiff:
        ...
        for nombre, fn_tdb, fn_tif in zip(NOMBRES, fns_tdb, fns_tif):
            ...
            casos.append({
                "caso": nombre, "frio_tdb_s": tf_tdb, "cal_tdb_s": tc_tdb,
                "frio_tif_s": tf_tif, "cal_tif_s": tc_tif, "factor": factor,
                "ram_tdb_mb": ram_tdb, "ram_tif_mb": ram_tif,
            })
        ...
        for nombre, fn_tdb in zip(NOMBRES, fns_tdb):
            ...
            casos.append({
                "caso": nombre, "frio_tdb_s": tf_tdb, "cal_tdb_s": tc_tdb,
                "frio_tif_s": None, "cal_tif_s": None, "factor": None,
                "ram_tdb_mb": ram_tdb, "ram_tif_mb": None,
            })

    if args.json_out:
        from bench_meta import machine_metadata
        payload = {
            "tipo": "vs_geotiff", "dataset": args.dataset,
            "tdb_mb": tdb_mb, "gtiff_mb": gtiff_mb,
            "reduccion_pct": (1 - tdb_mb / gtiff_mb) * 100,
            "have_gtiff": have_gtiff, "casos": casos,
            "maquina": machine_metadata(),
        }

...
Los tiempos anteriores se midieron en una única máquina. Para comprobar que las propiedades del sistema no dependen del hardware, las mismas consultas se ejecutaron en dos equipos de distinta clase: el ordenador de desarrollo (CPU de gama alta, con 16~GB disponibles bajo WSL2) y el servidor VPS de producción (menos núcleos, 23~GB y disco SSD). Sus características y los tiempos medidos se detallan en el Anexo~\ref{anexo:rendimiento-extra}. El ordenador resulta entre 1,3 y 1,7$\times$ más rápido por su CPU, pero lo relevante es que el orden relativo de los patrones de acceso se conserva en ambas máquinas: el comportamiento del índice disperso es consistente entre hardware distinto.

Se evaluó además la sensibilidad a la memoria disponible, repitiendo el \textit{benchmark} de consultas en el ordenador con la memoria del proceso limitada mediante \texttt{systemd-run} (\textit{cgroups}). El rendimiento se mantiene prácticamente constante (en torno a 80~s) desde 12~GB hasta 4~GB, y solo por debajo de ese umbral el proceso es abortado por falta de memoria. Este resultado confirma una decisión de diseño clave: al acumular sobre un \textit{grid} de tamaño fijo en lugar de materializar listas (sección~\ref{rendimiento-consultas}), el consumo de memoria queda acotado e independiente del volumen de celdas húmedas, lo que hace el rendimiento robusto hasta un umbral bajo de RAM y justifica la recomendación de al menos 16~GB en el servidor.

...
Los tiempos anteriores se midieron en una sola máquina. Para comprobar que las propiedades del sistema no dependen del hardware, las mismas consultas se ejecutaron en dos equipos de distinta clase: el ordenador de desarrollo (CPU de gama alta, 16~GB bajo WSL2) y el servidor VPS de producción (menos núcleos, 23~GB y disco SSD), cuyas características y tiempos se detallan en el Anexo~\ref{anexo:rendimiento-extra}. El ordenador es entre 1,3 y 1,7$\times$ más rápido por su CPU, pero lo relevante es que el orden relativo de los patrones de acceso se conserva en ambos: el comportamiento del índice disperso es consistente entre hardware distinto.

También se evaluó la sensibilidad a la memoria, repitiendo el \textit{benchmark} con la memoria del proceso limitada mediante \texttt{systemd-run} (\textit{cgroups}): el rendimiento se mantiene casi constante (en torno a 80~s) de 12~GB a 4~GB, y solo por debajo el proceso se aborta por falta de memoria. Esto confirma una decisión de diseño clave: al acumular sobre un \textit{grid} de tamaño fijo en lugar de materializar listas (sección~\ref{rendimiento-consultas}), el consumo de memoria queda acotado e independiente del volumen de celdas húmedas, lo que hace el rendimiento robusto hasta un umbral bajo de RAM y justifica recomendar al menos 16~GB en el servidor.

...
\item \textbf{Análisis:} El motor de consultas proporciona 24 consultas que operan directamente sobre el array TileDB sin exportación previa.
...
\begin{tikzpicture}[
    ...
    \node[consumer, below=14mm of tdb, xshift=-32mm] (queries) {Motor de consultas\\\scriptsize 24 consultas analíticas};
...
\begin{tikzpicture}[x=0.9cm, y=0.9cm, font=\small\sffamily]
  ...
  \node[gray!55, font=\tiny, align=center]
    at (\BX+\BW/2,\BY+0.18) {$\sim$17,2 GB por escenario};
...
La configuración final ahorra un 8,9\,\% de espacio con tiempos de lectura equivalentes, a cambio de un incremento mínimo del tiempo de carga que resulta asumible: la ingesta es un proceso puntual frente a la consulta continua de los datos. Las herramientas construidas sobre ella se presentan en el capítulo~\ref{herramientas-desarrolladas} y su validación y rendimiento en el capítulo~\ref{validacion-del-sistema}.

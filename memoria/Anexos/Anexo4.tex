...
El proceso ETL (\textit{geotiff\_to\_tiledb\_sparse.py}) transforma los ficheros GeoTIFF generados por Triton en un array TileDB sparse listo para consulta. Este anexo describe el proceso con el nivel de detalle necesario para reproducirlo o adaptarlo a otro simulador. La Figura~\ref{fig:etl-flow} muestra el diagrama de flujo completo.
...
\end{tikzpicture}
\caption{Diagrama de flujo del proceso ETL. Por cada escenario se iteran los 20 pasos temporales: se leen los cuatro ficheros GeoTIFF, se filtra por H $\geq$ 0,01 m y se escriben solo las celdas húmedas en el array TileDB sparse con compresión ByteShuffle+ZSTD-9. Al finalizar se verifica la integridad mediante muestreo aleatorio.}
\label{fig:etl-flow}
\end{figure}

\section{Parámetros de configuración}\label{d.1-parametros-de-configuracion}

Los tres parámetros que controlan el comportamiento del proceso ETL se definen como constantes al inicio del script y se describen en la Tabla~\ref{tab:etl-params}.
...
El programa llama a \texttt{create\_sparse\_array()} para crear el array con la siguiente configuración:
...
\item \textbf{Filtro de compresión:} \texttt{tiledb.ByteShuffleFilter()} seguido de \texttt{tiledb.ZstdFilter(level=9)}, elegidos tras el \textit{benchmark} del \hyperref[anexo-b.-tablas-de-resultados-completas]{Anexo~B} (reducción adicional del 8,9\,\% frente a ZSTD-1 sin penalización en lectura).
\end{itemize}

Si el array ya existe en disco, el programa lo elimina antes de recrearlo para evitar la acumulación de fragmentos de ejecuciones anteriores.

\section{Bucle de procesamiento por paso temporal}\label{d.3-bucle-por-paso}

El cuerpo principal del proceso ETL itera sobre los 20 pasos temporales en orden cronológico. Para cada paso \texttt{t}:
...
\item \textbf{Progreso.} El programa imprime por pantalla el número de celdas húmedas almacenadas y el tiempo empleado en cada paso.
\end{enumerate}

Este bucle se repite para los 20 pasos temporales del escenario; al completarlo, el array contiene exactamente un fragmento por paso y está listo para la fase de verificación.
...
Tras completar la escritura de los 20 fragmentos, el módulo de ingesta almacena en el propio array TileDB los metadatos geoespaciales necesarios para interpretar los datos y para la interoperabilidad con herramientas SIG, recogidos en la Tabla~\ref{tab:etl-metadata}.
...
\texttt{bounds}     & Extensión espacial en el sistema de referencia de coordenadas (CRS), en JSON \\
...
Estos metadatos permiten al motor de consultas reconstruir la georreferenciación sin necesidad de conservar los GeoTIFF originales, lo que hace posible desplegar el sistema en un servidor donde solo se transfieren los arrays TileDB (17--21 GB por escenario) y no los datos brutos ($\sim$235 GB). La clave \texttt{zero\_rule} garantiza la interoperabilidad: cualquier celda ausente en el array equivale a valor cero, convención necesaria para que la exportación a GeoTIFF produzca rásteres completos con \textit{nodata} correctamente asignado.
...
Una vez verificados estos tres requisitos, el programa puede ejecutarse con el siguiente comando.
...
Al finalizar la ingesta el programa imprime un resumen similar al siguiente (valores reales de datos1):
...
Resolución: 23.040 x 34.200 px  |  Franja: 5.000 filas

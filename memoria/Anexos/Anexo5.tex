...
El sistema expone tres variables de entorno para configurar las rutas de datos:

\begin{itemize}
\item \texttt{TRITON\_BASE\_URI}: directorio que contiene \texttt{triton\_results/} con los arrays TileDB. Usada por todos los componentes.
\item \texttt{TRITON\_GTIFF\_DIR}: directorio raíz con las carpetas de GeoTIFF fuente (\texttt{datos1/}, \texttt{datos2/}, \ldots). Solo necesaria para el ETL.
\item \texttt{TRITON\_OUTPUT\_DIR}: directorio donde se guardan las exportaciones a GeoTIFF. Opcional.
\end{itemize}

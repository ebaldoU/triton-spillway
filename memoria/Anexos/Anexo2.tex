...
Se evaluaron tres familias de filtros: ZSTD puro (niveles 1, 3, 5 y 9), ByteShuffle + ZSTD y BitShuffle + ZSTD (niveles 1, 3, 5, 7 y 9), así como la combinación DeltaFilter + ZSTD. Esta última no fue aplicable porque TileDB no acepta DeltaFilter sobre atributos de tipo float32. La Tabla~\ref{tab:g1-filtros} recoge los resultados completos.
...
C2. Ventana 50$\times$50 km & 3,1$\times$ TDB & 3,7$\times$ TDB & $\checkmark$ TDB gana \\
C3. Serie 1 píxel & 5,3$\times$ TIF & 5,4$\times$ TIF & $\checkmark$ TIF gana \\
...
\bottomrule
\end{tabularx}
\captionsetup{hypcap=false}\captionof{table}{Comparativa del factor de aceleración TileDB vs GeoTIFF entre los \textit{datasets} datos1 y datos2.}
...
\bottomrule
\end{tabularx}
\captionsetup{hypcap=false}\captionof{table}{Métricas globales por \textit{dataset} (datos1--datos4).}
\label{tab:metricas-datasets}
\end{center}

\clearpage
\section{Rendimiento en distintas máquinas}\label{anexo:rendimiento-extra}

Esta sección detalla la evaluación resumida en la sección~\ref{rendimiento-multimaquina}. La Tabla~\ref{tab:specs-maquinas} recoge las características de las dos máquinas empleadas, la Tabla~\ref{tab:multimaquina} compara el tiempo de las consultas en ambas y la Figura~\ref{fig:curva-ram} muestra la sensibilidad a la memoria disponible.

\begin{table}[ht]
\centering
\small
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{3.2cm}L>{\centering\arraybackslash}p{1.8cm}>{\centering\arraybackslash}p{1.6cm}}
\toprule
\textbf{Máquina} & \textbf{CPU} & \textbf{RAM (GB)} & \textbf{Disco} \\
\midrule
Ordenador (desarrollo) & AMD Ryzen 9 7900X (12 / 24 hilos) & 32 (16 WSL2) & SSD NVMe \\
...
La comparación se realiza sobre los tiempos de TileDB (los GeoTIFF originales se eliminan tras la ingesta); la reducción de almacenamiento frente a GeoTIFF se cuantifica en el capítulo~\ref{diseno-del-sistema}.
...
\toprule
\textbf{Consulta} & \textbf{Ordenador (s)} & \textbf{VPS (s)} \\
\midrule
...
Un último experimento caracteriza la sensibilidad del sistema a la memoria disponible, un aspecto crítico para el despliegue en servidores modestos. Para ello se repitió el \textit{benchmark} de consultas multi-paso sobre el dominio completo (escenario \texttt{datos1}) limitando progresivamente la memoria del proceso, desde 12 hasta 2~GB. La Figura~\ref{fig:curva-ram} muestra el tiempo total obtenido en cada nivel: entre 12 y 4~GB el rendimiento apenas varía (menos de un 9~\%), lo que confirma que el \textit{grid} acumulador de tamaño fijo mantiene el consumo de memoria acotado con independencia del número de celdas recorridas. Por debajo de ese umbral el comportamiento cambia de forma abrupta: con solo 2~GB el proceso agota la memoria y se aborta (OOM) antes de terminar la consulta, lo que sitúa en torno a 4~GB el mínimo de memoria para ejecutar las consultas multi-paso.
...
\end{axis}
\end{tikzpicture}
\caption{Tiempo del \textit{benchmark} de consultas al limitar la memoria del proceso. El rendimiento es estable hasta 4~GB; a 2~GB el proceso se aborta por agotamiento de memoria (marcado como OOM).}
\label{fig:curva-ram}
\end{figure}
